...
import type { Badge, CreateBadgeRequest, UpdateBadgeRequest, GrantRequest, BulkGrantRequest } from "@/lib/types/gamification";
import {
  useBadges,
  useCreateBadge,
  useUpdateBadge,
    ...
    if (editTarget?.id) {
      ...
      updateBadge.mutate(
        { id: String(editTarget.id), data },
        {
          ...
          onError: (err) => {
            toast.danger(err instanceof Error ? err.message : "Error al actualizar badge");
          },
        },
      );
    } else {
      ...
      createBadge.mutate(data, {
        ...
        onError: (err) => {
          toast.danger(err instanceof Error ? err.message : "Error al crear badge");
        },
      });
    }
  };
    ...
    if (isBulk) {
      ...
      bulkGrantBadge.mutate(
        { badgeId: String(grantTarget.id), data },
        {
          ...
          onError: (err) => {
            toast.danger(err instanceof Error ? err.message : "Error en otorgamiento masivo");
          },
        },
      );
    } else {
      ...
      grantBadge.mutate(
        { badgeId: String(grantTarget.id), data },
        {
          ...
          onError: (err) => {
            toast.danger(err instanceof Error ? err.message : "Error al otorgar badge");
          },
        },
      );
    }
  ...
  const handleRevoke = () => {
    ...
    revokeBadge.mutate(
      { badgeId: String(revokeTarget.id), data },
      {
        ...
        onError: (err) => {
          toast.danger(err instanceof Error ? err.message : "Error al revocar badge");
        },
      },
    );
  };
    ...
    if (catId) {
      updateBadgeCategory.mutate(
        ...
        },
        {
          ...
          onError: (err) => {
            toast.danger(err instanceof Error ? err.message : "Error al actualizar categoria");
          },
        },
      );
    } else {
      createBadgeCategory.mutate(
        ...
        },
        {
          ...
          onError: (err) => {
            toast.danger(err instanceof Error ? err.message : "Error al crear categoria");
          },
        },
      );
    }
